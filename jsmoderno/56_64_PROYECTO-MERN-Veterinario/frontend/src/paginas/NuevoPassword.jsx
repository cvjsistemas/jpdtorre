...
const NuevoPassword = () => {
    ...
    useEffect(()=>{
        const comprobartoken = async ()=>{
            try {
                await clienteAxios(`/veteribarios/olvide-password/${token}`);
    ...
    const handleSubmit = async(e)=>{
        ...
        try {
            ...
            const {data} = await clienteAxios.post(url,{password:password});
            console.log(data);
  ...
  return (
   <>
       <div>
             <h1 className="text-indigo-600 font-black text-6xl">Restablece tu password y no pierdas acceso <span className="text-black"> 
              tus Pacientes</span>
             </h1>
         </div>
